...
2. **连接硬件**：每个装置一个 USB-TTL，插入后设备管理器确认 COM 口（串口列表已自动过滤蓝牙设备，并内置 COM1–COM20 手动选项，覆盖枚举不到的设备）
...
| GET | `/api/gripper/ports` | 列出夹爪可用串口（过滤蓝牙，含 COM1–20 手动选项） |
...
| POST | `/api/heartbeat` | 页面心跳（前端 2s 一次，看门狗存活判据；日志中已过滤） |
| POST | `/api/client_exit` | 页面关闭告别信标，看门狗据此移除客户端 |
...
- **进度到 100% 后**：还有运动方程拟合阶段（约 10~60 秒，视数据量），进度条会显示"正在拟合运动方程"提示，属正常现象

## 注意事项

- YOLO 模型按需懒加载：启动秒开，首次用到某实验类型时才加载对应权重，之后常驻内存
- **叉掉网页后约 11 秒服务器自动停止**（页面卸载发告别信标 + 心跳看门狗：6 秒无请求保护窗 + 5 秒确认宽限；页面异常崩溃未发信标时最长约 95 秒兜底）。**刷新页面、切后台/最小化不会误关停**（前端主线程+Worker 双通道心跳，任意请求都会刷新活动时间戳）。多标签页时最后一个关闭才停；退出前电磁铁自动断电保护。`停止服务器.bat` 保留为手动兜底
- 桌面快捷方式「摆动实验分析平台」：双击启动服务并自动打开网页（等端口就绪后打开，不会报错），图标为 `icon.ico`（深海星空 + 霓虹摆球定制设计）
